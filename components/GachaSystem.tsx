...
const GachaSystem: React.FC<Props> = ({ credits, starJade, onSpendCredits, onSpendStarJade, onAddItem, onAddCharacter, onBack, allCharacters, isTutorial, trackStat }) => {
  ...
  const hsrChars = CHARACTERS.filter(c => c.game === 'Honkai: Star Rail' || c.game === 'Original');
  const genshinChars = CHARACTERS.filter(c => c.game === 'Genshin Impact');

  const doPull = (forceSSR: boolean = false) => {
     const rand = Math.random();
     if (isTutorial && forceSSR) return CHARACTERS.find(c => c.id === 'firefly')!;
